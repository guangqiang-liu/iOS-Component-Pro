...
@implementation WLPickerViewManager

+ (WLPickerViewManager *)shareManger {
    static WLPickerViewManager *manager = nil;
...
- (WLDatePickerView *)datePicker {
    if (!_datePicker) {
        _datePicker = [WLDatePickerView new];
    }
    return _datePicker;
}

- (WLPickerView *)pickView {
    if (!_pickView) {
        _pickView = [WLPickerView new];
    }
    return _pickView;
}

- (WLAddressPickerView *)addressPicker {
    if (!_addressPicker) {
        _addressPicker = [WLAddressPickerView new];
    }
