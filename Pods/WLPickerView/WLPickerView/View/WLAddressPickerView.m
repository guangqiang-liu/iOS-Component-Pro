...
@interface WLAddressPickerView() <UIPickerViewDelegate, UIPickerViewDataSource, NSXMLParserDelegate>

@property (nonatomic, strong) NSXMLParser *parser;

@property (nonatomic, strong) UIView *bgView;
@property (nonatomic, strong) NSMutableArray<WLAddressModel *> *dataArr;
...
@implementation WLAddressPickerView
    ...
    self.toolBar.commitBlock = ^ {
        if (commitBlock) {
            ...
            if (weakSelf.dataArr.count > 0) {
                WLAddressModel *addressModel = weakSelf.dataArr[weakSelf.selectedIndex_province];
                CityModel *cityModel;
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary<NSString *,NSString *> *)attributeDict {
    if ([elementName isEqualToString:@"province"]) {
        WLAddressModel *model = [[WLAddressModel alloc] initWithDictionary:attributeDict];
...
- (NSString *)searchByKey:(NSString *)key valueName:(NSString *)valueName type:(NSString *)type {
    ...
    WLAddressModel *addressModel = (WLAddressModel *)[self searchModelInArr:_dataArr key:arr[0] type:type];
    if (addressModel) {
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {
        WLAddressModel *addressModel = self.dataArr[row];
        return addressModel.name;
    } else if (component == 1) {
        WLAddressModel *addressModel = self.dataArr[self.selectedIndex_province];
